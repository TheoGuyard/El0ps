...
class ConvexRegfunc(BaseRegfunc):
    ...
    def value_scalar(self, i: int, lmbd: float, x: float) -> float:
        if x == 0.0:
            return 0.0
        elif 0.0 < x <= self.penalty.param_limit_pos_scalar(i, lmbd):
...
class BnbBoundingSolver:
        ...
        for _ in range(self.maxiter_outer):
            ...
            if dualpruning or screening or simpruning or peeling:
                ...
                if simpruning:
                    ...
                    node.simpruning_S0 += np.sum(S0) - old_S0
                    node.simpruning_S1 += np.sum(S1) - old_S1
                if peeling:
                    self.peeling(
                        self.datafit,
                        self.regfunc,
    ...
    def peeling(
        ...
        for i in np.flatnonzero(Sb):
            ...
            if t > 0:
                ...
                if x[i] != 0.0:
                    ...
                    x[i] = 0.0
                    flag = True
                regfunc.penalty.update_upper_bound_scalar(i, 0.)
                regfunc.penalty.update_lower_bound_scalar(i, 0.)
            else:
                c = (t / v[i])
                if v[i] < 0.0:
                    x_ub_i = np.maximum(c + regfunc.penalty.x_lb[i], 1e-16)
                    if x_ub_i < regfunc.penalty.x_ub[i]:
                        regfunc.penalty.update_upper_bound_scalar(i, x_ub_i)
                elif v[i] > 0.0:
                    x_lb_i = np.minimum(c + regfunc.penalty.x_ub[i], -1e-16)
        ...
        for i in np.flatnonzero(S1):
            if v[i] < 0.0:
                c = (dv - ub) / v[i]
                x_ub_i = np.maximum(regfunc.penalty.x_lb[i] + c, 1e-16)
                if x_ub_i < regfunc.penalty.x_ub[i]:
                        regfunc.penalty.update_upper_bound_scalar(i, x_ub_i)
            elif v[i] > 0.0:
                c = (dv - ub) / v[i]
                x_lb_i = np.minimum(regfunc.penalty.x_ub[i] - c, -1e-16)
                if x_lb_i > regfunc.penalty.x_lb[i]:
                    regfunc.penalty.update_lower_bound_scalar(i, x_lb_i)
