import argparse
import pathlib
import random
import shutil
import string
import subprocess
import yaml

src_path = "~/Documents/Github/el0ps"
dst_path = "thguyard@igrida-frontend.irisa.fr:/srv/tempdd/thguyard/repos"
logs_dir = "/srv/tempdd/thguyard/logs"

base_dir = pathlib.Path(__file__).parent.parent.parent.absolute()
experiments_dir = base_dir.joinpath("experiments")
script_dir = experiments_dir.joinpath("scripts")
run_file = "run.sh"
run_path = script_dir.joinpath(run_file)

experiments = [
    {
        "name": "perfprofile",
        "walltime": "05:00:00",
        "besteffort": False,
        "setups": [
            {
                "expname": "perfprofile",
                "dataset": {
                    "datatype": "synthetic",
                    "datafit_name": "Leastsquares",
                    "penalty_name": penalty,
                    "k": k,
                    "m": 100,
                    "n": 300,
                    "rho": 0.1,
                    "snr": 10.0,
                    "normalize": True,
                },
                "solver_names": ["el0ps", "sbnb", "l0bnb", "gurobi", "mosek"],
                "solver_options": {
                    "time_limit": 3600.0,
                    "rel_tol": 1.0e-4,
                    "int_tol": 1.0e-8,
                    "verbose": False,
                },
            }
            for penalty in ["Bigm", "BigmL2norm"]
            for k in [5, 7, 9]
        ],
    },
    {
        "name": "atp",
        "walltime": "04:30:00",
        "besteffort": False,
        "setups": [
            {
                "expname": "atp",
                "dataset": {
                    "datatype": "atp",
                    "datafit_name": "Leastsquares",
                    "penalty_name": "NeglogTriangular",
                    "nnz_proba": 0.1,
                    "m": 10,
                    "n": 30,
                    "snr": 10.0,
                    "t": t,
                },
                "solver_names": ["el0ps", "gurobi", "mosek"],
                "solver_options": {
                    "time_limit": 600.0,
                    "rel_tol": 1.0e-4,
                    "int_tol": 1.0e-8,
                    "verbose": False,
                },
                "path_options": {
                    "lmbd_ratio_max": 1.0e-0,
                    "lmbd_ratio_min": 1.0e-3,
                    "lmbd_ratio_num": 10,
                    "warm_start": True,
                },
            }
            for t in [1, 2, 3]
        ],
    },
]


def make_run():
    stream = "\n".join(
        [
            "#!/bin/sh",
            "expname=$1",
            "repeats=$2",
            "for i in $(seq 1 $repeats);",
            "do",
            "   oarsub -S {}/$expname/oar.sh".format(script_dir),
            "done",
        ]
    )
    with open(run_path, "w") as file:
        file.write(stream)
    subprocess.run("chmod u+x {}".format(run_path), shell=True)


def make_oar(experiment):
    # Create the experiment dir
    experiment_dir = script_dir.joinpath(experiment["name"])
    experiment_dir.mkdir()

    # Create the args file (remove old ones)
    args_path = experiment_dir.joinpath("args.txt")
    if args_path.is_file():
        args_path.unlink()

    # Create the oar file
    stream = "\n".join(
        [
            "#!/bin/sh",
            "#OAR -n el0ps-{}".format(experiment["name"]),
            "#OAR -O {}/el0ps-{}.%jobid%.output".format(
                logs_dir, experiment["name"]
            ),
            "#OAR -E {}/el0ps-{}.%jobid%.error".format(
                logs_dir, experiment["name"]
            ),
            "#OAR -l walltime={}".format(experiment["walltime"]),
            "#OAR -t besteffort" if experiment["besteffort"] else "",
            "#OAR --array-param-file {}".format(args_path),
            "set -xv",
            "source $HOME/.bashrc",
            "$PYTHONBIN {}/{}/exp.py $*".format(
                experiments_dir, experiment["name"]
            ),
        ]
    )
    oar_path = experiment_dir.joinpath("oar.sh")
    with open(oar_path, "w") as file:
        file.write(stream)
    subprocess.run("chmod u+x {}".format(oar_path), shell=True)

    # Create the setups dir
    setups_dir = experiment_dir.joinpath("setups")
    setups_dir.mkdir()

    # Generate the setups and the arg file
    for setup in experiment["setups"]:
        setup_name = "".join(
            random.choice(string.ascii_lowercase) for _ in range(10)
        )
        setup_file = "{}.yaml".format(setup_name)
        setup_path = pathlib.Path(setups_dir, setup_file)

        with open(setup_path, "w") as file:
            yaml.dump(setup, file)

        with open(args_path, "a") as file:
            file.write(str(setup_path))
            file.write("\n")


def make_install():
    cmd_str = "$PYTHONBIN -m pip install -q -e .[exp]"
    subprocess.run(cmd_str, shell=True)


def oar_send():
    print("send el0ps")
    cmd_str = " ".join(
        [
            "rsync -am",
            "--exclude '.git'",
            "--exclude '.venv'",
            "--exclude '**/results/*.pickle'",
            "--exclude '**/saves/*.csv'",
            "{} {}".format(src_path, dst_path),
        ]
    )
    subprocess.run(cmd_str, shell=True)


def oar_receive():
    for experiment in experiments:
        print("receive {}".format(experiment["name"]))
        results_src_path = pathlib.Path(
            dst_path, "srglm", "experiments", experiment["name"], "results/*"
        )
        results_dst_path = pathlib.Path(
            experiments_dir, experiment["name"], "results"
        )
        cmd_str = "rsync -am {} {}".format(results_src_path, results_dst_path)
        subprocess.run(cmd_str, shell=True)


def oar_make():
    oar_clean()
    print("make install")
    make_install()
    print("make run")
    make_run()
    for experiment in experiments:
        print("make oar {}".format(experiment["name"]))
        make_oar(experiment)


def oar_clean():
    if run_path.is_file():
        print("clean run.sh")
        run_path.unlink()
    for experiment in experiments:
        experiment_dir = script_dir.joinpath(experiment["name"])
        if experiment_dir.is_dir():
            print("clean {}".format(experiment["name"]))
            shutil.rmtree(experiment_dir)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "oar_cmd", choices=["make", "clean", "send", "receive"]
    )
    args = parser.parse_args()

    if args.oar_cmd == "make":
        oar_make()
    elif args.oar_cmd == "clean":
        oar_clean()
    elif args.oar_cmd == "send":
        oar_send()
    elif args.oar_cmd == "receive":
        oar_receive()
    else:
        raise ValueError(f"Unknown command {args.oar_cmd}.")
