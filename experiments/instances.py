...
def get_data_synthetic(
    model="linear", k=5, m=100, n=200, rho=0.5, snr=10.0, normalize=False
):
    x = synthetic_x(k, n)
...
def get_data_libsvm(dataset_name="bodyfat"):
    import ssl
...
def get_data_openml(dataset_id=217, dataset_target="activity"):
...
def get_data_uciml(dataset_id=53):
...
def get_data_hardcoded(dataset_name="lattice"):
    dataset_path = pathlib.Path(__file__).parent.joinpath(
        "datasets", dataset_name + ".npz"
    )
    with np.load(dataset_path) as dataset_file:
        A = dataset_file["A"]
        y = dataset_file["y"]
        x = None if "x" not in dataset_file.files else dataset_file["x"]
    return A, y, x


def process_data(
    datafit_name, A, y, x_true, interactions=False, normalize=False
):
...
def calibrate_objective(datafit_name, penalty_name, A, y, x_true=None):
    bindings = {
        ...
        "Logcosh": "SquaredError",
        "Logistic": "Logistic",
        "Squaredhinge": "SquaredHinge",
        "Kullbackleibler": "Logistic",
        "Bigm": "L0",
        "BigmL1norm": "L0L1",
        "BigmL2norm": "L0L2",
        "L1norm": "L0L1",
    ...
    cvfit = l0learn.cvfit(
        np.array(A.reshape(*A.shape, order="F")),
        y,
        bindings[datafit_name],
        bindings[penalty_name],
        intercept=False,
        num_gamma=1 if bindings[penalty_name] == "L0" else 25,
    ...
    best_gamma = np.maximum(best_gamma, 1e-8 / m)
