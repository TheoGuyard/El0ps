...
def onerun_sensibility(config_path, nosave=False):
    ...
    for param_name, param_values in config["dataset"]["variations"].items():
        for param_value in param_values:
            ...
            print(
                "      x shape: {}".format(
                    None if x_true is None else x_true.shape
                )
            )
            ...
            print(
                "      lratio: {}".format(
                    lmbd / compute_lmbd_max(datafit, penalty, A)
                )
            )
            for (
                penalty_param_name,
                penalty_param_value,
            ) in penalty.params_to_dict().items():
                print(
                    "      {}\t: {}".format(
                        penalty_param_name, penalty_param_value
                    )
                )
            ...
            solvers = {
                solver_name: get_solver(
                    solver_name, config["solvers"]["solvers_opts"]
                )
                for solver_name in config["solvers"]["solvers_name"]
            }
            ...
            for solver_name, solver in solvers.items():
                if can_handle(
                    ...
                    result = solver.solve(problem)
                    solver.options.time_limit = config["solvers"][
                        "solvers_opts"
                    ]["time_limit"]
                    ...
                    print("        status    : {}".format(result.status))
                    print(
                        "        obj. value: {}".format(result.objective_value)
                    )
                    print("        non-zeros : {}".format(result.n_nnz))
                    print("        solve time: {}".format(result.solve_time))
                    print("        iter count: {}".format(result.iter_count))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "experiment", choices=["perfprofile", "sensibility", "regpath"]
    )
